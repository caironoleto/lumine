...
class Lumine_Export_MySQL extends ILumine_Export {
	...
	/**
	 * Mщtodo especializado que irс realizar a criaчуo das tabelas no banco
	 * @author Hugo Ferreira da Silva
	 * @link http://www.hufersil.com.br
	 */
	...
	public function create()
	{
		...
		// para cada chave estrangeira, criaremos o SQL de geraчуo do Foreign Key
		...
		// ok, agora que temos as definiчѕes de tabela, vamos criс-las no banco
		// щ sѓ executar uma a uma
	...
	/**
	 * Cria as definiчѕes (DLL) das tabelas (CREATE TABLE)
	 * @author Hugo Ferreira da Silva
	 * @link http://www.hufersil.com.br
	 */
	public function getTablesDefinition()
	{
		...
		foreach( $this->classList as $obj )
		{
			...
			// vamos olhar agora as definiчѕes M-T-M
			...
			foreach( $relations as $relation )
			{
				if( $relation['type'] == Lumine_Base::MANY_TO_MANY && !array_key_exists($relation['table'], $mtm) )
				{
					...
					// coloca a definiчуo da tabela mtm no array
					$this->tables[] = $tabledef;
	...
	/**
	 * Cria os comandos SQL para gerar os эndices
	 * @author Hugo Ferreira da Silva
	 * @link http://www.hufersil.com.br
	 * @return void
	 */
